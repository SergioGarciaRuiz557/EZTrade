...
class PortfolioServiceTest {
    ...
    @Mock
    private CashAccountRepositoryPort cashAccountRepository;
    ...
    @Test
    @DisplayName("BUY abre posicion y actualiza cash")
    void buy_opens_position_and_updates_cash() {
        ...
        given(cashAccountRepository.findByOwner("user@demo.com")).willReturn(Optional.empty());
        given(cashAccountRepository.save(any(CashAccount.class))).willAnswer(i -> i.getArgument(0));

        portfolioService.handle(event);

        verify(positionRepository, times(1)).save(any(Position.class));
        verify(cashAccountRepository, times(1)).save(any(CashAccount.class));
    ...
    void sell_reduces_position_and_publishes_reduced_event() {
        ...
        given(cashAccountRepository.findByOwner("user@demo.com"))
                .willReturn(Optional.of(CashAccount.rehydrate("user@demo.com", BigDecimal.ZERO)));
        given(cashAccountRepository.save(any(CashAccount.class))).willAnswer(i -> i.getArgument(0));
